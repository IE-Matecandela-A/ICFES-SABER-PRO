...
const AuthModule = {
    ...
    async register(name, email, school, grade, sex, password) {
        // Simple hash or encoding for ID
        const userId = btoa(email.toLowerCase().trim()).replace(/=/g, '');
        const newUser = { email, id: userId, name, school, grade, sex, password };
...
const AuthUI = {
    ...
    async handleRegister() {
        ...
        if (!name || !email || !password || !school || !grade || !sex) {
        ...
        const success = await AuthModule.register(name, email, school, grade, sex, password);
