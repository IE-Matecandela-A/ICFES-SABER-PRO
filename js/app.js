...
const ResultsEngine = {
    ...
    calculate(questions, answers, startTime, studentName) {
        ...
        if (isInvalidated) {
            ...
            if (typeof VirtualTeacherModule !== 'undefined') {
                VirtualTeacherModule.speak(
                    '⏰ ¡Ey! Completaste el simulacro en menos de 10 minutos. Eso es muy rápido. Este resultado NO será registrado, no sumará XP ni aparecerá en ranking. La próxima vez tómate tu tiempo y lee bien cada pregunta. 📖',
                    'serious'
                );
            } else {
                NotificationModule.show('Simulacro completado en menos de 10 minutos. No sumará XP ni aparecerá en el ranking.', 'warning', 6000);
            }
        } else {
            // Save to history
            try {
                ...
                const historyItem = {
                    ...
                    suspicious: (timeSpent > 0 && questions.length > 0 && (timeSpent / questions.length) < 30)
                };
...
const AdminPanelModule = {
                ...
                .map(([firebaseKey, userObj]) => {
                    ...
                    history.forEach(h => {
                        if (!h.isSpam && !h.invalidated && h.timeSpent > 0 &&
                            ...
                            totalSessionSeconds += h.timeSpent;
                        }
                    });
    ...
    renderStudents() {
        ...
        container.innerHTML = filtered.map(student => {
            ...
            const missingAreas = ['Matemáticas', 'Lectura Crítica', 'Ciencias Naturales', 'Sociales', 'Inglés']
...
const VirtualTeacherModule = {
    ...
    speak(message, mood = 'neutral', actions = []) {
        ...
        if (actions.length === 0 && !this.isTouring) {
            this.hideTimeout = setTimeout(() => this.hide(), 8000);
        }
    },
