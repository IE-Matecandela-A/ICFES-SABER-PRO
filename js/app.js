    ...
    calculate(questions, answers, startTime, studentName) {
        ...
        try {
            ...
            if (typeof GamificationModule !== 'undefined') {
                GamificationModule.addXP(50 + (correct * 10)); // Base 50 + 10 per correct answer
            }
                ...
                <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 16px; overflow: hidden; transition: all 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                    ...
                    <div style="padding: 16px 20px; display: flex; align-items: center; justify-content: space-between; cursor: pointer;" 
                        ...
                        <div style="display: flex; align-items: center; gap: 16px; flex: 1;">
                            ...
                            <div>
                                <div style="font-weight: 700; color: var(--color-text); font-size: 1.05rem;">${student.name}</div>
                                <div style="color: var(--color-text-muted); font-size: 0.85rem; display: flex; align-items: center; gap: 6px;">
                        ...
                        <!-- Col 3: Last Date & Expand Icon -->
                        <div style="display: flex; align-items: center; gap: 16px; width: 140px; justify-content: flex-end;">
                    ...
                    <div class="hidden" style="border-top: 1px solid var(--color-border); background: rgba(0,0,0,0.02); padding: 20px;">
                        ...
                        <div style="margin-top: 16px; display: flex; gap: 12px; justify-content: flex-end;">
                            <button style="padding: 6px 12px; font-size: 0.85rem; border-radius: 8px; border: 1px solid var(--color-border); background: var(--color-surface); color: var(--color-text); cursor: pointer;">
                                <span class="material-icons-round" style="font-size: 14px; vertical-align: middle;">visibility</span> Ver Historial Categórico
                            </button>
                            <button style="padding: 6px 12px; font-size: 0.85rem; border-radius: 8px; border: none; background: rgba(99,102,241,0.1); color: var(--color-primary); cursor: pointer; font-weight: 600;">
                                <span class="material-icons-round" style="font-size: 14px; vertical-align: middle;">mail</span> Enviar Recomendación
                            </button>
                        </div>
    ...
    _updateActiveTime() {
        if (!this.isPaused && this.lastTickTime) {
            ...
            if (elapsed > 0) {
                ...
                // Grant 1 XP for every active minute (silent grant, no notification spam)
                if (typeof GamificationModule !== 'undefined' && AuthModule.currentUser) {
                    ...
                    if (currentMinute > this.lastXpMinute) {
                        ...
                        GamificationModule.currentXP += minutesGained;
...
const GamificationModule = {
    levels: [
        { level: 1, title: 'Novato 11°', xpReq: 0 },
        { level: 2, title: 'Estudiante Activo', xpReq: 200 },
        { level: 3, title: 'Aspirante', xpReq: 600 },
        { level: 4, title: 'Analista Crítico', xpReq: 1200 },
        { level: 5, title: 'Promesa Saber', xpReq: 2500 },
        { level: 6, title: 'Cerebro Destacado', xpReq: 5000 },
        { level: 7, title: 'Intelectual', xpReq: 9000 },
        { level: 8, title: 'Erudito', xpReq: 14000 },
        { level: 9, title: 'Genio Académico', xpReq: 20000 },
        { level: 10, title: 'Maestro Nacional', xpReq: 25000 }
    ],
    currentXP: 0,
    ...
    async init(user) {
        ...
        try {
            ...
            if (data && data.xp) {
                this.currentXP = parseInt(data.xp, 10) || 0;
            }
    ...
    calculateLevels() {
        ...
        for (let i = 0; i < this.levels.length; i++) {
            if (this.currentXP >= this.levels[i].xpReq) {
        ...
        this.currentLevelObj = current;
        this.nextLevelObj = next;
    },
    ...
    async addXP(points) {
        ...
        try {
            await fetch(`https://plataforma-icfes-13421-default-rtdb.firebaseio.com/users/${AuthModule.currentUser.id}/gamification.json`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ xp: this.currentXP, lastUpdated: Date.now() })
            });
            if (typeof updateUserUI === 'function') updateUserUI();
            ...
            levelHtml = `
                <div style="margin-top: 2px;">
                    <div style="font-size: 0.70rem; font-weight: 700; color: var(--color-primary); display: flex; align-items: center; gap: 4px; line-height: 1; margin-bottom: 3px;">
                        ...
                        Lvl ${level.level} <span style="color: var(--color-text-muted); font-weight: 500;">| ${level.title}</span>
                    </div>
...
const GlobalResultsModule = {
    ...
    renderGlobalResults() {
        ...
        container.innerHTML = `<div style="text-align: center; padding: 40px; color: var(--color-text-muted);"><div style="font-size: 2rem; margin-bottom: 12px; animation: pulse 1.5s infinite;">🔄</div>Cargando resultados globales...</div>`;
        this._fetchAndRenderResults();
    },
        ...
        if (merged.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 60px; color: var(--color-text-muted); background: var(--color-surface); border-radius: 16px; border: 1px dashed var(--color-border);">
                    <div style="font-size: 3rem; margin-bottom: 16px;">📊</div>
                    <div style="font-size: 1.1rem; font-weight: 600; margin-bottom: 8px;">No hay resultados registrados</div>
                    <p>Sé el primero en completar un simulacro y aparecer en el ranking.</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = `
            <div class="glass" style="overflow-x: auto; padding: 0; border: 1px solid var(--color-border); border-radius: 16px;">
                <table style="width: 100%; border-collapse: collapse; min-width: 600px;">
                    <thead style="background: var(--color-surface-2); border-bottom: 1px solid var(--color-border);">
                        <tr>
                            <th style="padding: 16px; text-align: left; font-size: 0.75rem; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase;">Puesto</th>
                            <th style="padding: 16px; text-align: left; font-size: 0.75rem; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase;">Estudiante</th>
                            <th style="padding: 16px; text-align: left; font-size: 0.75rem; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase;">Prueba</th>
                            <th style="padding: 16px; text-align: left; font-size: 0.75rem; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase;">Puntaje</th>
                            <th style="padding: 16px; text-align: center; font-size: 0.75rem; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase;">Fecha</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${merged.map((item, index) => {
            const date = new Date(item.date || item.timestamp).toLocaleDateString('es-CO');
            const scoreColor = item.score >= 80 ? 'var(--color-success)' : (item.score >= 60 ? 'var(--color-accent)' : 'var(--color-danger)');
            ...
            return `
                            <tr style="border-bottom: 1px solid var(--color-border);" class="result-row">
                                <td style="padding: 16px; font-weight: 800; font-size: 1.2rem;">${medal}</td>
                                <td style="padding: 16px; font-weight: 600; font-size: 1rem;">${item.studentName || 'Anónimo'}</td>
                                <td style="padding: 16px; font-size: 0.9rem;">${item.title || 'Simulacro'}</td>
                                <td style="padding: 16px;">
                                    <div style="font-size: 1.1rem; font-weight: 800; color: ${scoreColor};">${item.icfesScore || Math.round((item.correct / item.total) * 500)} / 500</div>
                                </td>
                                <td style="padding: 16px; text-align: center; font-size: 0.85rem; color: var(--color-text-muted);">
                                    ${date}
                                </td>
                            </tr>
                            `;
